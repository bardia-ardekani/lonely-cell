...
struct player1 * split(int size , int flag[size][size] ,char map[size][size] , struct player1 * head , char name[20] , int x , int y , int symbol);
char *randstring(size_t length);
void createCellPlayer1(struct player1 ** head, int size ,int flag[size][size] , char map[size][size] , int p , int symbol);
void insertCellPlayer1(struct player1 * head , int size , char map[size][size]);
struct player1 * createNode(char name[] , int x , int y , int energy , int p , int symbol);
...
void save( struct player1 * p1 , struct player1 * p2 , int p ,  int size , int energyArr[size][size] ,char map[size][size]);
struct player1 * load1(struct player1 *head , int size , int flag[size][size] , char map[size][size] ,int energyArr[size][size] , int p , int symbol);
struct player1 * load2(struct player1 *head , int size , int flag[size][size] , char map[size][size] ,int energyArr[size][size] , int p , int symbol);
...
struct player1 * split(int size , int flag[size][size] ,char map[size][size] , struct player1 * head , char name[20] , int x , int y , int symbol)
{
    ...
    if(emptyCell == 0) {printf("there is no empty cells\n");Sleep(500); return head;} ///there is no empty cells
    ...
    case 1:
        ...
        printf("%s\n" , rands1);
        insert(&head , createNode(rands1 , current->x , current->y , 40 , 1 , symbol));
        break;
    case 2:
        printf("name your mother cell : ");
        scanf("%s" , name1);
        insert(&head , createNode(name1 , current->x , current->y , 40 , 1 , symbol));
        break;
    }
    ...
    case 1:
        ...
        printf("%s\n" , rands2);
        insert(&head , createNode(rands2 , nx , ny , 1 , 40 , symbol));
        flag[nx][ny] = 1;
    ...
    case 2:
        ...
        scanf("%s" , name2);
        insert(&head , createNode(name2 , nx , ny , 1 , 40 , symbol));
        flag[nx][ny] = 1;
        deleteNode(&head , current->x , current->y);
        break;
    }
...
void createCellPlayer1(struct player1 ** head, int size ,int flag[size][size] , char map[size][size] , int p , int symbol)
{
    ...
    for(int i = 0 ; i < cellCount ; i++)
    {
        ...
        case 1:
            ...
            printf("%s\n" , rands);
            insert(&(*head) , createNode(rands , x , y , 0 , p , symbol));
            break;
        case 2:
            printf("name your cell : ");
            scanf("%s" , nameStr);
            insert(&(*head) , createNode(nameStr , x , y , 0 , p , symbol));
            break;
        }
    }
}
...
void insertCellPlayer1(struct player1 * head , int size , char map[size][size])
{
    ...
    if(current->symbol == 1) {printf("X");}
    if(current->symbol == 2) {printf("Y");}
    current = current->next;
    }
...
void save( struct player1 * p1 , struct player1 * p2 , int p ,  int size , int energyArr[size][size] ,char map[size][size])
{
    ...
    if(save1 == NULL) {printf("ERROR!"); return;}
    if(save2 == NULL) {printf("ERROR!"); return;}
    fwrite(&size , sizeof(int) , 1 , maps);
    struct player1 * curr1 = p1;
    struct player1 * curr2 = p2;
    while(curr1!=NULL)
    {
        fprintf(save1 ,"%s %d %d %d\n" , curr1->cellName , curr1->x , curr1->y , curr1->energy);
        curr1 = curr1->next;
    }
    while(curr2!=NULL)
    {
        fprintf(save2 ,"%s %d %d %d\n" , curr2->cellName , curr2->x , curr2->y , curr2->energy);
        curr2 = curr2->next;
    }
    for(int i = 0 ; i < size ; i++)
     {
...
struct player1 * load1(struct player1 *head , int size , int flag[size][size] , char map[size][size] ,int energyArr[size][size] , int p , int symbol)
{
    ...
    if(n == 0)
    {
        createCellPlayer1(&head , size , flag , map , p , symbol);
        return head;
    }
    FILE *save = fopen("save1.txt" , "r");
    FILE *res = fopen("resourse.txt" , "r");
    while(1)
    {
        if(feof(save)) break;
        fscanf(save , "%s %d %d %d\n" , name , &x , &y , &energy);
        insert(&head , createNode(name , x , y , energy , p , symbol));
        printList(&head);
    }
    printf("hey");
    for(int i = 0 ; i < size ; i++)
